...
public class ShortestDistanceAfterRoadAdditionQueriesi3243 {
    ...
    //S.C : O(V+E)
    private Map<Integer, List<Integer>> adj1 = new HashMap<>();
    ...
    private int bfs(int n) {
        ...
        while (!queue.isEmpty()) {
            ...
            while (size-- > 0) {
                ...
                for (int neighbor : adj1.getOrDefault(node, new ArrayList<>())) {
                    if (!visited[neighbor]) {
    ...
    public int[] shortestDistanceAfterQueries(int n, int[][] queries) {
        ...
        for (int i = 0; i < n - 1; i++) {
            adj1.computeIfAbsent(i, k -> new ArrayList<>()).add(i + 1);
        }
        ...
        for (int i = 0; i < k; i++) {
            int u = queries[i][0];
            int v = queries[i][1];
            adj1.computeIfAbsent(u, k1 -> new ArrayList<>()).add(v);
            res[i] = bfs(n);
        }
